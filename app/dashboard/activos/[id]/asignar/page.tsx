...
export default function AsignarActivoPage({
  ...
  useEffect(() => {
    async function loadData() {
      ...
      // 1. Obtener perfil para conocer el client_id
      ...
      // 2. Obtener activo y lista de empleados
      ...
      if (assetData) {
        ...
        // 3. Si el activo está asignado, consultar la asignación activa actual
  ...
  const handleAssign = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!asset || !assignedTo || !currentUserId) return;

    setIsSubmitting(true);

    // 1. Crear registro en asset_assignments
    ...
    if (assignError) {
      alert("Error al registrar asignación: " + assignError.message);
    ...
    if (assetError) {
      alert("Error al actualizar estado del equipo: " + assetError.message);
      setIsSubmitting(false);
      return;
    }
  ...
  const handleReturn = async (e: React.FormEvent) => {
    ...
    const { error: returnError } = await supabase
      ...
      .update({
        ...
        notes: notesIn ? `[Devolución]: ${notesIn}` : null,
      })
      .eq("id", activeAssignment.id);

    if (returnError) {
      alert("Error al registrar devolución: " + returnError.message);
      setIsSubmitting(false);
      return;
    }
    ...
    if (assetError) {
      alert("Error al actualizar el estado del equipo: " + assetError.message);
      setIsSubmitting(false);
      return;
    }
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      {asset.status === "asignado" ? (
        <form onSubmit={handleReturn} className="p-8 bg-white/40 dark:bg-gray-900/40 backdrop-blur-xl rounded-3xl border border-white/60 dark:border-gray-700/50 shadow-[0_8px_32px_0_rgba(31,38,135,0.1)] space-y-6">
        ...
        <form onSubmit={handleAssign} className="p-8 bg-white/40 dark:bg-gray-900/40 backdrop-blur-xl rounded-3xl border border-white/60 dark:border-gray-700/50 shadow-[0_8px_32px_0_rgba(31,38,135,0.1)] space-y-6">
